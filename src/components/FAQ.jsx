    ...
    <section ref={sectionRef} className="bg-[#121212] py-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-12 text-center animate-on-scroll">
          ...
          <a
            href={isRestaurantOpen ? `https://wa.me/${restaurantInfo.whatsapp}` : '#'}
            target={isRestaurantOpen ? '_blank' : undefined}
            rel={isRestaurantOpen ? 'noopener noreferrer' : undefined}
            onClick={handleWhatsAppClick}
            className={`inline-flex items-center space-x-2 px-8 py-4 rounded-full font-semibold transition-all ${
              isRestaurantOpen
                ? 'bg-[#25D366] text-white hover:bg-[#20BA5A] shadow-lg shadow-green-500/20'
          ...
          >
            ...
            <span>{isRestaurantOpen ? `Escribinos al ${restaurantInfo.phone}` : 'CERRADO'}</span>
          </a>
        </div>
        ...
        {isClosedModalOpen && (
          ...
          >
            <div
              className="bg-[#1A1A1A] rounded-2xl p-8 max-w-md w-full border border-gray-800 shadow-2xl"
